...
class SessionManager:
    ...
    CLAUDE_MODELS = {
        'Anthropic Models': [
            ('sonnet', 'Claude Sonnet (Latest)', ['claude-sonnet', 'claude-sonnet-4.5', 'sonnet-4.5']),
            ('haiku', 'Claude Haiku (Latest)', ['claude-haiku', 'claude-haiku-4.5', 'haiku-4.5']),
            ('opus', 'Claude Opus (Latest)', ['claude-opus', 'claude-opus-4.5', 'opus-4.5']),
        ]
    }
    ...
    def __init__(self, config_file: str | None = None):
        ...
        self.opencode_session_storage = Path.home() / ".local" / "share" / "opencode" / "storage" / "session" / "global"
    ...
    def _load_agents_config(self, config_file: str | None = None) -> dict:
        ...
        if config_file is None:
            ...
            config_path = Path(config_file) if config_file else Path.cwd() / "agents.json"
        ...
        if not config_path.exists():
            print(f"[Warning] Agents config file not found at {config_path}. Using empty agents.", file=sys.stderr)
            return {}
        ...
        try:
            with open(config_path, 'r') as f:
                ...
                for agent in config.get('agents', []):
                    name = agent.get('name')
                    if not name:
                        print(f"[Warning] Agent entry missing 'name' field", file=sys.stderr)
                        continue
                    agents[name] = {
                        'path': agent.get('path', ''),
        ...
        try:
            ...
            if not models:
                 ...
                 if found_fallbacks:
                     ...
                     models = [m for m in loose_match if 'gpt' in m or 'claude' in m or 'gemini' in m]
            ...
            for m in models:
                cat = 'Other Models'
                if 'claude' in m.lower(): cat = 'Claude Models'
                elif 'gpt' in m.lower() or re.match(r'^o\d', m.lower()): cat = 'GPT Models'
                elif 'gemini' in m.lower(): cat = 'Google Models'
    ...
    def fetch_opencode_models(self) -> dict:
        ...
        try:
            ...
            if result.returncode != 0:
                print(f"[Error] opencode models failed (exit {result.returncode}): {result.stderr}", file=sys.stderr)
                return {}
            ...
            print("[Error] opencode models command timed out after 30s", file=sys.stderr)
            return {}
        except Exception as e:
    ...
    def get_or_create_session_data(self, n8n_session_id: str) -> dict:
        ...
        self.save_session_map(session_map)
        print(f"[Session] Created new session: {default_data['session_id']} (N8N: {n8n_session_id})", file=sys.stderr)
    ...
    def strip_metadata(self, text: str, runtime: str) -> str:
        ...
        if runtime == 'copilot':
            ...
            for line in lines:
                ...
                if skip_banner and ('█' in clean_line or '▄' in clean_line or (clean_line.strip() == '' and len(result) == 0)):
                    continue
                skip_banner = False
                
                # Skip stats
                if any(k in clean_line.lower() for k in ['tokens used:', 'total cost:', 'session id:', 'commands:', 'positionals:', 'options:']):
                    continue
...
{prompt}"""
    ...
    def run_claude(self, prompt: str, model: str, agent: str, session_id: str | None, resume: bool, n8n_session_id: str) -> str:
        ...
        if resume and session_id:
            ...
            print(f"[Session] Resuming Claude session: {session_id}", file=sys.stderr)
             ...
             print(f"[Session] Starting new Claude session: {session_id}", file=sys.stderr)
        else:
        ...
        try:
            ...
            if result.returncode != 0:
                print(f"[Error] Claude command failed (exit {result.returncode}): {output}", file=sys.stderr)
    ...
    def get_most_recent_session_id(self, runtime: str, agent: str = 'devops') -> str | None:
        ...
        try:
            if runtime == 'copilot':
                files = sorted(self.session_state_dir.glob("*.jsonl"), key=lambda p: p.stat().st_mtime, reverse=True)
...
"""
            ...
            if argument == 'list' or argument.startswith('list '):
                ...
                if not model_id: return f"Unknown model '{model_name}' for runtime {current_runtime}"
        ...
        if current_runtime == 'copilot':
            if can_resume:
                output = self.run_copilot(prompt, model, agent, session_id, True, n8n_session_id)
            else:
                output = self.run_copilot(prompt, model, agent, None, False, n8n_session_id)
            ...
            if can_resume:
                output = self.run_opencode(prompt, model, agent, session_id, True, n8n_session_id)
                # Check for session loss / resource not found
                if "Resource not found" in output or "NotFoundError" in output:
                     print(f"[Session] Session {session_id} lost/corrupted. Starting new session.", file=sys.stderr)
                     output = self.run_opencode(prompt, model, agent, None, False, n8n_session_id)
...
def main():
    if len(sys.argv) < 2:
        print("Usage: agent-manager.py <prompt> [session_id] [config_file]", file=sys.stderr)
        sys.exit(1)
